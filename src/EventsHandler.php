...
use XPocketMC\CustomNPCs\{Entity\CustomNPCs, Form\CustomizeMain};
...
class EventsHandler implements Listener
{
    ...
    public function EntityDamageByEntityEvent(EntityDamageByEntityEvent $event)
    {
        ...
        if ($NPC instanceof CustomNPCs and $player instanceof Player) {
            ...
            if ($this->main->spawnedForFistTime($NPC)) {
                if ($player->hasPermission('CustomNPCs.permission')) { # Send tip to enable editor mode.
     ...
     * @param string $tag
     */
    public function execute(Player $player, CustomNPCs $NPC, string $tag)
    {
        # Cooldown stuff.
    ...
    {
        if ($event->getEntity() instanceof CustomNPCs) {
