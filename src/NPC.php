...
class NPC extends PluginBase
{
    ...
    public function onEnable()
    {
        ...
        Entity::registerEntity(CustomNPCs::class, true);
     ...
     * @return bool
     */
    public static function isset(CustomNPCs $NPC, string $value, $Tag = null): bool
    {
     ...
     * @return array
     */
    public static function get(CustomNPCs $NPC, string $Tag): array
    {
        $result = [];
     ...
     * @param null $Tag
     */
    public static function remove(CustomNPCs $NPC, string $value, $Tag = null)
    {
     ...
     * @param null $Tag
     */
    public static function add(CustomNPCs $NPC, string $value, $Tag = null)
    {
        $TheTag = $Tag ?? 'Commands';
            ...
            case 'npc':
                if ($player instanceof Player) {
                    if (count($args) < 2) {
                        if (isset($args[0])) {
                            /* NPC Editor */
                            if (strtolower($args[0]) === "edit" and $player->hasPermission('CustomNPCs.permission')) {
    ...
    public function spawn(Player $player)
    {
        ...
        $NPC = new CustomNPCs ($player->getLevel(), $NBT);
     ...
     * @return boolean
     */
    public function spawnedForFistTime(CustomNPCs $npc): bool
    {
        return (is_null($npc->namedtag->getCompoundTag("Commands")) and is_null($npc->namedtag->getCompoundTag("Interactions")));
    }

    public function getNPCCooldown(CustomNPCs $npc): float
    {
